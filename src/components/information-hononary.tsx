...
import { LoaderWithComplete } from './loader-with-complete';
...
export function InformationHonorary() {
  ...
  const [selectedMes, setSelectedMes] = useState<string>('all'); // controla o Select
  const [mes, setMes] = useState<number | null>(null); // controla o filtro
  ...
  const totals = useMemo(
    () => ({
      ...
      percHonorario: filtrados.reduce(
        (sum, r) => sum + r.percentageHonorary,
        0
      ),
    }),
    [filtrados]
  ...
  if (isError)
    return <p className="text-center mt-10">Erro ao carregar dados.</p>;
                    ...
                    <tbody>
                      {paginados.map((r, i) => (
                        <tr key={i} className="border-t">
                          ...
                          <td className="p-2">
                            {r.honorary.toLocaleString('pt-BR', {
                              style: 'currency',
                              currency: 'BRL',
                            })}
                          </td>
                          <td className="p-2">
                            {r.compensation.toLocaleString('pt-BR', {
                              style: 'currency',
                              currency: 'BRL',
                            })}
                          </td>
                          ...
                          <td className="p-2">
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={() => navigate(`/honorario/${r.id}`)}
                            >
                              Detalhes
                            </Button>
                          </td>
