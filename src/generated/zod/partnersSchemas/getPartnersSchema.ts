/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import { z } from 'zod';

/**
 * @description Default Response
 */
export const getPartners200Schema = z.array(
  z.object({
    id: z.string(),
    name: z.string().nullable(),
    cpfOrCnpj: z.string().nullable(),
    city: z.string().nullable(),
    state: z.string().nullable(),
    commission: z.number().nullable(),
    portal: z.string().nullable(),
    channelHead: z.string().nullable(),
    regional: z.string().nullable(),
    coordinator: z.string().nullable(),
    agent: z.string().nullable(),
    indicator: z.string().nullable(),
    contract: z.string().nullable(),
    phone: z.string().nullable(),
    email: z.string().nullable(),
    responsible: z.string().nullable(),
  })
);

export type GetPartners200Schema = z.infer<typeof getPartners200Schema>;

export const getPartnersQueryResponseSchema = z.lazy(
  () => getPartners200Schema
);

export type GetPartnersQueryResponseSchema = z.infer<
  typeof getPartnersQueryResponseSchema
>;
